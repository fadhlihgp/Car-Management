...
const FormSearch = () => {
	...
	const fetchData = async () => {
		...
		const filterData = cars.filter((res: any) => {
			return (
					...
					<div className='d-flex justify-content-around'>
						...
						<div style={{ width: "21.75%"}}>
							...
							<Form.Select value={selectedTime} onChange={handleTimeChange} disabled={disable}>
								...
								{listTime.map(({show, value}, index: number) => {
			...
			{showCar && dataCars && (
				<div style={listCarStyle}>
					{dataCars.map(( { carType, price, carTransmission, capacity, year, description, pictureUrl, id} ) => {
						return <SearchCarCard key={id} type={carType.name} capacity={capacity} rentPerDay={price} transmission={carTransmission.name} year={year} description={description} imgUrl={pictureUrl} id={id} />;
					})}
				</div>
			)}
